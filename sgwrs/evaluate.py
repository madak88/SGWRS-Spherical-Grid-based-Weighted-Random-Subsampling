...
from pointpillars.utils import keep_bbox_from_image_range_2, \
...
def do_eval(det_results, gt_results, CLASSES, saved_path):
    ...
    for id in ids:
        ...
        if len(det_result['name'])<1:

            iou2d_v = iou_bev_v = iou3d_v = np.zeros((len(gt_result['name']), 1), dtype=np.float32)
            ...
            for difficulty in [0, 1, 2]:
                ...
                for id in ids:
                    ...
                    if len(det_result['name'])<1:
                        ...
                        total_det_ignores.append([-1])
                        total_scores.append(np.array([0.0], dtype=np.float32))
                        total_det_alpha.append(np.array([0.0], dtype=np.float32))
                    ...
                    for i, id in enumerate(ids):
                        ...
                        if eval_type == 'bbox_2d':
                            ...
                            if len(dc_bboxes) > 0:
                                ...
                                if len(det_results[id]['name'])<1: det_bboxes = np.zeros((0, 4))
...
def main(args):
    ...
    if not args.no_cuda:
        model = PointPillars(nclasses=args.nclasses, ds_mode=args.ds_mode, ds_num=args.ds_num).cuda()
        if not args.ckpt.startswith("pretrained_legacy"):
            model.load_state_dict(torch.load(args.ckpt)['state_dict'])
        else:
            model.load_state_dict(torch.load(args.ckpt))
    else:
        model = PointPillars(nclasses=args.nclasses, ds_mode=args.ds_mode, ds_num=args.ds_num)
        if not args.ckpt.startswith("pretrained_legacy"):
            model.load_state_dict(torch.load(args.ckpt, map_location=torch.device('cpu'))['state_dict'])
        else:
    ...
    with torch.no_grad():
        ...
        for i, data_dict in enumerate(tqdm(val_dataloader)):
            ...
            if args.save_ds_pcd: save_info = {'path': saved_path, 'batched_idx': data_dict['batched_idx'], 'batched_calib_info': data_dict['batched_calib_info']}
            else: save_info = {}
            ...
            for j, result in enumerate(batch_results):
                ...
                crop_image_corners = data_dict['batched_img_info'][j]['image_corners']
                idx = int(data_dict['batched_idx'][j])
                ...
                if result == ([],[],[]):
                    ...
                    result_filter = keep_bbox_from_image_range_2(result, tr_velo_to_cam, r0_rect, P2, crop_image_corners)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt', default='pretrained_legacy/epoch_160.pth', help='your checkpoint for kitti')
    parser.add_argument('--saved_path', default='_results_tmp', help='your saved path for predicted results')
    parser.add_argument('--batch_size', type=int, default=8)
    parser.add_argument('--num_workers', type=int, default=4)
    parser.add_argument('--nclasses', type=int, default=3)
    parser.add_argument('--no_cuda', action='store_true', help='whether to use cuda')
    parser.add_argument("--ds_mode", default="none", choices=["cn", "rs", "sgwrs", "fps"], help=( 
    ...
    parser.add_argument('--ds_num', type=int, default=1024)
    parser.add_argument('--save_ds_pcd', action='store_true')
