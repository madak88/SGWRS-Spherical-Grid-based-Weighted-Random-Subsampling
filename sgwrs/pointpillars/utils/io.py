...
def write_calib(file_path, calib_dict, fmt="%.12e"):
    def _require(key):
        if key not in calib_dict:
            raise KeyError(f"Missing key in calib_dict: '{key}'")
        return np.asarray(calib_dict[key])
    ...
    def _to_3x4(M):
        ...
        if M.shape == (4, 4):
            return M[:3, :4]
        raise ValueError(f"Unsupported matrix shape (expected 3x4 or 4x4, got {M.shape})")
    ...
    def _to_3x3(M):
        ...
        if M.shape == (4, 4):
            return M[:3, :3]
        raise ValueError(f"Unsupported matrix shape (expected 3x3 or 4x4, got {M.shape})")

    P0 = _to_3x4(_require("P0"))
    P1 = _to_3x4(_require("P1"))
    P2 = _to_3x4(_require("P2"))
    P3 = _to_3x4(_require("P3"))

    R0_rect = _to_3x3(_require("R0_rect"))
    Tr_velo_to_cam = _to_3x4(_require("Tr_velo_to_cam"))
    Tr_imu_to_velo = _to_3x4(_require("Tr_imu_to_velo"))

    def _line(name, M):
        vals = " ".join(fmt % v for v in M.astype(np.float32).ravel())
        return f"{name}: {vals}\n"
    ...
    with open(file_path, "w") as f:
        ...
        f.write(_line("P2", P2))
        f.write(_line("P3", P3))
        f.write(_line("R0_rect", R0_rect))
        f.write(_line("Tr_velo_to_cam", Tr_velo_to_cam))
        f.write(_line("Tr_imu_to_velo", Tr_imu_to_velo))


def read_label(data, fromFile=True):
    if fromFile:
