...
from casnet.models.ds_model_cp import CascadeAttentionBasedSampling
from casnet.settings.chamfer_distance.chamfer_distance import get_cd_loss
...
from utils import fps, hybrid_spherical_downsample

from pointpillars.utils import write_points, write_calib
import os
...
class PointPillars(nn.Module):
    def __init__(self,
        ...
        self.ds_num = ds_num
        if ds_mode == 'cn': self.casnet_layer = CascadeAttentionBasedSampling(ds_num = ds_num, hard=True)
    ...
    def forward(self, batched_pts, mode='test', batched_gt_bboxes=None, batched_gt_labels=None, save_info={}):
...
# = Downsampling part =================================================================================================
        ...
        # Random Sampling
        if self.ds_mode == 'rs':
            tensor_batched_pts = torch.stack(batched_pts, dim=0) # (B, N, C)
            B, N, C = tensor_batched_pts.shape
            idx = torch.rand(B, N, device=tensor_batched_pts.device).argsort(dim=1)[:, :self.ds_num]  # (B, M)
            ds_tensor_batched_pts = tensor_batched_pts.gather(1, idx.unsqueeze(-1).expand(-1, -1, C))  # (B, M, C)
            ds_batched_pts = list(ds_tensor_batched_pts) # B x (M, C)

        # Spherical Grid-based Weighted Random Subsampling
        elif self.ds_mode == 'sgwrs': 
            # print('TODO: Make it tensor and remove \'for\'')
            ds_ratio = batched_pts[0].shape[0] / self.ds_num
            for i, pts in enumerate(batched_pts):
                batched_pts[i], _ = hybrid_spherical_downsample(pts, self.ds_num, ds_ratio=ds_ratio)
                # Quick problem check: 
                if batched_pts[i].shape[0] != self.ds_num: print(batched_pts[i].shape)
            ds_batched_pts = batched_pts


        # Farthest Point Sampling
        elif self.ds_mode == 'fps': 
            tensor_batched_pts = torch.stack(batched_pts, dim=0)
            ds_tensor_batched_pts = fps(tensor_batched_pts, self.ds_num)
            ds_batched_pts = list(ds_tensor_batched_pts)

        # CAS-Net
        elif self.ds_mode == 'cn': 
            tensor_batched_pts = torch.stack(batched_pts, dim=0).permute(0, 2, 1)
            tensor_sampling_matrices, cos_loss = self.casnet_layer(tensor_batched_pts[:, :3])
            tensor_batched_pts = tensor_batched_pts.permute(0, 2, 1)
            ds_tensor_batched_pts = tensor_sampling_matrices.bmm(tensor_batched_pts)
            ds_loss = get_cd_loss(ds_tensor_batched_pts, tensor_batched_pts)
            if mode == 'train':
                casnet_loss = ds_loss + cos_loss
                casnet_loss.backward()
            ds_batched_pts = list(ds_tensor_batched_pts)
        ...
        # Save calibration data and downsampled point cloud for later visualization
        if save_info: 
            save_ds_pcd_path = os.path.join(save_info['path'], 'ds_pcd')
            save_calib_path = os.path.join(save_info['path'], 'calib')
            os.makedirs(save_ds_pcd_path, exist_ok=True)
            os.makedirs(save_calib_path, exist_ok=True)
            for i, act_ds_pts in enumerate(ds_batched_pts):
                idx = int(save_info['batched_idx'][i])
                calib = save_info['batched_calib_info'][i]
                write_calib(os.path.join(save_calib_path, f'{idx:06d}.txt'), calib)
                write_points(act_ds_pts.detach().cpu().numpy(), os.path.join(save_ds_pcd_path, f'{idx:06d}.bin'))
...
# =====================================================================================================================
        ...
        #                              coors_batch: (p1 + p2 + ... + pb, 1 + 3), 
        #                              num_points_per_pillar: (p1 + p2 + ... + pb, ), (b: batch size)
        pillars, coors_batch, npoints_per_pillar = self.pillar_layer(ds_batched_pts)
        ...
        if mode == 'train' or mode == 'eval':
            ...
            return ds_loss, cos_loss, bbox_cls_pred, bbox_pred, bbox_dir_cls_pred, anchor_target_dict
            # return 0, 0, bbox_cls_pred, bbox_pred, bbox_dir_cls_pred, anchor_target_dict
        elif mode == 'val':
