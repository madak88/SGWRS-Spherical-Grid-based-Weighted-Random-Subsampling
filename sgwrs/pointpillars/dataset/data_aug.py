...
from pointpillars.utils import bbox3d2bevcorners, box_collision_test, read_points, \
    remove_pts_in_bboxes, limit_period, iterative_crop, get_points_num_in_bbox, bbox_lidar2camera
...
def crop_again(data_dict, data_info):

    lidar_points = data_dict['pts']
    calib_dict = data_dict['calib_info']
    image_info = data_dict['image_info']
    crop_info = data_info['crop']

    cropped_lidar_points, _, _ = iterative_crop(
        points=lidar_points, 
        r0_rect=calib_dict['R0_rect'], 
        tr_velo_to_cam=calib_dict['Tr_velo_to_cam'], 
        P2=calib_dict['P2'],
        image_shape=image_info['image_shape'],
        segments = crop_info['segments'],
        part=crop_info['part'],
        target=crop_info['target']
    )

    data_dict['pts'] = cropped_lidar_points

    tmp = bbox_lidar2camera(data_dict['gt_bboxes_3d'], calib_dict['Tr_velo_to_cam'], calib_dict['R0_rect'])

    dim = tmp[:, 3:6]   # lhw
    loc = tmp[:, :3]    # camera(xyz)
    rot = tmp[:, 6]     # yaw

    new_num_points_in_gt = get_points_num_in_bbox(
    ...
    for i, act_num_points_in_gt in enumerate(new_num_points_in_gt):
        if act_num_points_in_gt == 0: targets.append(i)

    data_dict['gt_bboxes_3d'] = np.delete(data_dict['gt_bboxes_3d'], targets, axis=0)
    data_dict['gt_labels'] = np.delete(data_dict['gt_labels'], targets, axis=0)
    data_dict['gt_names'] = np.delete(data_dict['gt_names'], targets, axis=0)

    return data_dict

def data_augment(CLASSES, data_root, data_dict, data_aug_config, data_info):
    '''
    CLASSES: dict(Pedestrian=0, Cyclist=1, Car=2)
    data_root: str, data root
    data_dict: dict(pts, gt_bboxes_3d, gt_labels, gt_names, difficulty)
    ...
    data_dict = crop_again(data_dict, data_info)
