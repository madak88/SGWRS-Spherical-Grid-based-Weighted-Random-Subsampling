...
def main(args):
    ...
    if not args.no_cuda:
        pointpillars = PointPillars(nclasses=args.nclasses, ds_mode=args.ds_mode, ds_num=args.ds_num).cuda()
    else:
        pointpillars = PointPillars(nclasses=args.nclasses, ds_mode=args.ds_mode, ds_num=args.ds_num)
    loss_func = Loss()
    ...
    checkpoint_path = os.path.join(args.saved_path, 'checkpoints', 'epoch_{}.pth'.format(args.resume_epoch))
    ...
    for epoch in range(args.resume_epoch, args.max_epoch):
        ...
        for i, data_dict in enumerate(tqdm(train_dataloader)):
            ...
            ds_loss, cos_loss, bbox_cls_pred, bbox_pred, bbox_dir_cls_pred, anchor_target_dict = \
            ...
            loss_dict = loss_func(ds_loss=ds_loss,
                                  cos_loss=cos_loss,
        ...
        if (epoch + 1) % args.ckpt_freq_epoch == 0:
            ...
            torch.save({
                'state_dict': pointpillars.state_dict(), 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--saved_path', default='_pillar_logs_tmp')
